...
disp_M = Model(inputs=inputs, outputs=SDN(inputs))
warped = SpatialDeformer3D(localization_net=disp_M, output_size=(input_shape[0], input_shape[1], input_shape[2]), \
                           input_shape=input_shape)(inputs)

sdn = Model(inputs=inputs, outputs=[warped, disp_M(inputs)])
...
history = sdn.fit_generator(gen_train, steps_per_epoch = len(train_list)/batch_size, \
                            epochs = epochs, use_multiprocessing = True, verbose=1, \
